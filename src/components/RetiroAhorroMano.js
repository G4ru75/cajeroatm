...
const RetiroAhorroMano = ({ onVolver }) => {
  ...
  const validarCuenta = (valor) => {
    ...
    let resultado = numeros.slice(0, 11);
    
    // Validar primer dígito (0 o 1)
    if (resultado.length > 0 && resultado[0] !== '0' && resultado[0] !== '1') {
      resultado = '';
    }
    
    // Validar segundo dígito (debe ser 3)
    if (resultado.length > 1 && resultado[1] !== '3') {
  ...
  const handleRetirar = () => {
    ...
    const validacionLimites = validarLimitesRetiro(monto, 'ahorroMano');
    ...
    if (resultado.exito) {
      ...
      descontarBilletes(resultado.distribucion, 'Ahorro a la Mano', cuenta);
      ...
      {paso === 1 && (
        <div className="paso">
          <div className="form-group">
            ...
            />
            <small>
              {cuenta.length}/11 dígitos - Debe iniciar con 0 o 1, segundo dígito debe ser 3
            </small>
          </div>
      ...
      {paso === 2 && (
        <div className="paso">
          ...
          <div className="limites-info">
            <h4>📋 Límites de Retiro Ahorro a la Mano:</h4>
            <p>• Mínimo: {formatearMoneda(LIMITES_RETIRO.ahorroMano.minimo)}</p>
            <p>• Máximo por retiro: <strong>{formatearMoneda(LIMITES_RETIRO.ahorroMano.maximo)}</strong></p>
          </div>
...
export default RetiroAhorroMano;
